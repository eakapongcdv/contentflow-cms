...
export default async function ListPage(){
  const items = await prisma.brand.findMany({ orderBy: { sort: "asc" } });
  return (<div className="card p-6">
    <div className="flex items-center justify-between"><h1 className="text-xl font-semibold capitalize">brand</h1>
      <Link href="/admin/brand/new" className="btn btn-primary">New</Link></div>
    <table className="w-full text-sm mt-4"><thead><tr>
      ...
      {items.map(i => (<tr key={i.id} className="border-t">
        ...
        <td className="p-2"><Link className="underline" href={`/admin/brand/$${i.id}`}>Edit</Link></td>
      </tr>))}
    </tbody></table></div>);
}
