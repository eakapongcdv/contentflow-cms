...
export default function Form({ initial, isNew }: any){
  ...
  async function onSubmit(e:any){ e.preventDefault(); setLoading(true);
    ...
    const res = await fetch(`/api/admin/infoCard${isNew?"":`/${initial.id}`}`, { method:isNew?"POST":"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) });
    if (res.ok) router.push("/admin/infoCard"); setLoading(false);
  }
  async function onDelete(){ if(!initial?.id) return; if(!confirm("Delete?")) return; await fetch(`/api/admin/infoCard/${initial.id}`,{method:"DELETE"}); router.push("/admin/infoCard"); }
  return (<div className="card p-6 max-w-2xl">
    <h1 className="text-xl font-semibold mb-4">{isNew?"Create":"Edit"} infoCard</h1>
    <form onSubmit={onSubmit} className="grid gap-3">
      <label className="label">title</label><input className="input" name="title" defaultValue={(initial?.title as any) ?? ""} /><label className="label">description</label><textarea className="input" name="description" defaultValue={(initial?.description as any) ?? ""} />
