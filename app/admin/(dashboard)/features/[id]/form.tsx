...
export default function Form({ initial, isNew }: any){
  ...
  async function onSubmit(e:any){ e.preventDefault(); setLoading(true);
    ...
    const res = await fetch(`/api/admin/feature${isNew?"":`/${initial.id}`}`, { method:isNew?"POST":"PUT", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body) });
    if (res.ok) router.push("/admin/feature"); setLoading(false);
  }
  async function onDelete(){ if(!initial?.id) return; if(!confirm("Delete?")) return; await fetch(`/api/admin/feature/${initial.id}`,{method:"DELETE"}); router.push("/admin/feature"); }
  return (<div className="card p-6 max-w-2xl">
    <h1 className="text-xl font-semibold mb-4">{isNew?"Create":"Edit"} feature</h1>
