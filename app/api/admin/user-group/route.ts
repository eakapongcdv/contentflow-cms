...
export async function GET() {
  ...
  let can = () => false as boolean;
  if (me.role === "ADMIN" && !websiteId) {
    can = () => true;
  } else {
    let rules: any[] = [];
    if (me.role === "ADMIN") rules = [{ resource: "*", action: "ALL", effect: "ALLOW" }];
    else if (websiteId) rules = await getEffectiveRulesForUser({ userId: me.id, userRole: me.role, websiteId });
  ...
  if (!websiteId) {
    // ADMIN without website: return all groups (or empty)
